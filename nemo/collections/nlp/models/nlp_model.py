...
class NLPModel(ModelPT, Exportable):
    ...
    def __init__(self, cfg: DictConfig, trainer: Trainer = None, no_lm_init=False):
        ...
        if cfg.get("tokenizer"):
            ...
            if (
                ...
                if (
                    (hasattr(self, "tokenizer") and self.tokenizer is None)
        ...
        if cfg.get("language_model") and not no_lm_init:
            if cfg.get("language_model").get("nemo_file"):
                nemo_file = self.register_artifact(
                    "language_model.nemo_file", cfg.language_model.nemo_file
                )
    ...
    def register_artifact(
        ...
        return super().register_artifact(
            config_path, src, verify_src_exists=verify_src_exists
        )
    ...
    def register_bert_model(self):
        ...
        if self.bert_model is not None:
            ...
            if isinstance(self.bert_model, BertModule):
                ...
                encoder_config_src = os.path.join(
                    NEMO_NLP_TMP, encoder_config_path + ".json"
                )
                self.bert_model.config.to_json_file(
                    encoder_config_src
                )  # name requested by jarvis team
                self.register_artifact(
                    "language_model.config_file", encoder_config_src
                )  # for .nemo
            # MegatronBertModel's superclass is NLP model, hence can't check for isinstance of self.bert_modelel
            elif (
                hasattr(self, "pretrained_model_name")
                and "megatron" in self.pretrained_model_name
            ):
                if self.pretrained_model_name in MEGATRON_CONFIG_MAP:
                    output_config = MEGATRON_CONFIG_MAP[self.pretrained_model_name][
                        "config"
                    ]
                    if output_config is not None:
                        encoder_config_path = (
                            self.pretrained_model_name + "_encoder_config"
                        )
                        encoder_config_src = os.path.join(
                            NEMO_NLP_TMP, encoder_config_path + ".json"
                        )
                        ...
                        logging.info(
                            f"For {self.pretrained_model_name}, set the config_file in the YAML file"
                        )
                else:
                    logging.info(
    ...
    def _register_vocab_from_tokenizer(
        ...
        if self.tokenizer is None:
            ...
            if isinstance(self.tokenizer, AutoTokenizer):
                ...
                self.register_artifact(
                    config_path=vocab_dict_config_path, src=vocab_json_src
                )
                ...
                # create vocab file
                vocab_file_src = os.path.join(
                    hash_path, VOCAB_FILE_NAME[tokenizer_name]
                )
                cfg.vocab_file = vocab_file_src
                self.register_artifact(
                    config_path=vocab_file_config_path, src=vocab_file_src
                )
            else:
                logging.info(
    ...
    def _make_nemo_file_from_folder(filename, source_dir):
        return super(NLPModel, NLPModel)._make_nemo_file_from_folder(
            filename, source_dir
        )
        ...
        try:
            ...
            if hasattr(cfg, "tokenizer"):
                if cfg.tokenizer.get("tokenizer_model") is not None:
                    model.register_artifact(
                        "tokenizer.tokenizer_model", cfg.tokenizer.tokenizer_model
                    )
            ...
            if hasattr(cfg, "encoder_tokenizer"):
                ...
                if cfg.encoder_tokenizer.get("vocab_file") is not None:
                    model.register_artifact(
                        "encoder_tokenizer.vocab_file", cfg.encoder_tokenizer.vocab_file
                    )
                if cfg.encoder_tokenizer.get("merge_file") is not None:
                    model.register_artifact(
                        "encoder_tokenizer.merge_file", cfg.encoder_tokenizer.merge_file
                    )
            ...
            if hasattr(cfg, "decoder_tokenizer"):
                ...
                if cfg.decoder_tokenizer.get("vocab_file") is not None:
                    model.register_artifact(
                        "decoder_tokenizer.vocab_file", cfg.decoder_tokenizer.vocab_file
                    )
                if cfg.decoder_tokenizer.get("merge_file") is not None:
                    model.register_artifact(
                        "decoder_tokenizer.merge_file", cfg.decoder_tokenizer.merge_file
                    )
