...
class DuplexDecoderModel(NLPModel):
    ...
    def input_example(self):
        ...
        decoder_input_ids = torch.randint(low=0, high=2048, size=(2, 16), device=sample.device)
        attention_mask = torch.randint(low=0, high=1, size=(2, 16), device=sample.device)
        labels = torch.randint(low=0, high=1, size=(2, 16), device=sample.device)

        return tuple([input_ids, decoder_input_ids, attention_mask, labels])
    ...
    def _infer(
        ...
        print("generated_texts, ids, sequence_toks_scores", generated_texts, generated_ids, sequence_toks_scores)
