...
class DuplexTextNormalizationModel(nn.Module):
    ...
    def _infer(self, sents: List[str], inst_directions: List[str], processed=False):
        ...
        if not processed:
            ...
            print("first processed sents : ", sents)
            sents = [self.decoder.processor.tokenize(x).split() for x in sents]
            #sents_tmp2 = "".join(sents_tmp1).split(" ")
            #sents = list()
            #sents.append(sents_tmp2)
            print("second processed sents : ", sents)
        else:
            sents = [x.split() for x in sents]
            print("not processed sents : ", sents)
        ...
        print("output_spans : ", output_spans)
