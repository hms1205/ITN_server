...
class ThutmoseTaggerModel(NLPModel):
    ...
    #        input_ids = torch.randint(low=0, high=2048, size=(1, 256), device=sample.device)
    #        segment_ids = torch.randint(low=0, high=1, size=(1, 256), device=sample.device)
    #        input_mask = torch.randint(low=0, high=1, size=(1, 256), device=sample.device)
    #        input_dict = {
    #            "input_ids": input_ids,
    ...
    def __init__(self, cfg: DictConfig, trainer: Trainer = None) -> None:
        ...
        # add STT unit tokens
        self.new_tokens = []
        with open("models/new_vocab.txt", "r", encoding="utf8") as f:
            lines = f.readlines()
            for line in lines:
                self.new_tokens.append(line.strip())
                logging.info(f"token append .. {line.strip()}")
        self.token_length = self.builder._tokenizer.vocab_size
        self.builder._tokenizer.add_tokens(self.new_tokens)
        logging.info(
            f"token append Done!, distil token length: {self.token_length}, added token length: {len(self.new_tokens)}, all: {self.token_length + len(self.new_tokens)}"
        )

        self.bert_model.resize_token_embeddings(
            self.token_length + len(self.new_tokens)
        )

    @typecheck()
    def forward(self, input_ids, input_mask, segment_ids):
        # self.bert_model.resize_token_embeddings(self.token_length + len(self.new_tokens))
        src_hiddens = self.bert_model(
            input_ids=input_ids, token_type_ids=segment_ids, attention_mask=input_mask
        )
        # print("src_hiddens:", src_hiddens)
    ...
    def _infer(self, sents: List[str]) -> List[List[int]]:
        ...
        print("sents :", sents)
        print("infer_datalayer :", infer_datalayer)
        ...
        print("semiotic_logits :", semiotic_logits)
        ...
        for i, sent in enumerate(sents):
            ...
            print("tag_logits, i :", i, tag_logits[i])
            print("torch.argmax :", torch.argmax(tag_logits[i], dim=-1))
            print("example :", example)
            print("tag_preds :", tag_preds)
            print("semiotic_preds :", semiotic_preds)

            # this mask is required by get_token_labels
            example.features["labels_mask"] = (
                [0] + [1] * (len(semiotic_preds) - 2) + [0]
            )
            print('example.features["labels_mask"] :', example.features["labels_mask"])
            example.features["tag_labels"] = tag_preds
            print('example.features["tag_labels"] :', example.features["tag_labels"])
            example.features["semiotic_labels"] = semiotic_preds
            print(
                'example.features["semiotic_labels"] :',
                example.features["semiotic_labels"],
            )
            tags = [
            ...
            print("example.get_token_labels :", example.get_token_labels("tag_labels"))
            print("tags :", tags)
            print("semiotic_labels :", semiotic_labels)
    ...
    def setup_training_data(self, train_data_config: Optional[DictConfig]):
        if not train_data_config or not train_data_config.data_path:
            ...
            self._train_dl = None
            return
        logging.info(f"setup_training_data!")
    ...
    def setup_validation_data(self, val_data_config: Optional[DictConfig]):
        if not val_data_config or not val_data_config.data_path:
            ...
            return
        logging.info(f"setup_validation_data!")
    ...
    def _setup_dataloader_from_config(self, cfg: DictConfig, data_split: str):
        ...
        # dataloader next 인덱스 호출부분 file size에 맞게 수정 필요
        dl = torch.utils.data.DataLoader(
            dataset=dataset,
            batch_size=cfg.batch_size,
            shuffle=cfg.shuffle,
            collate_fn=dataset.collate_fn,
            pin_memory=True,  # dataset cfg -> pin_memory
        )
    ...
    def _setup_infer_dataloader(
        ...
        """
        print("_setup_infer_dataloader queries :", queries)
