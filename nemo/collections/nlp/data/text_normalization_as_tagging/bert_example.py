...
def generate_example(builder, filename, infer):
    examples_count = 0
    for source, target, semiotic_info in yield_sources_and_targets(filename):
        example = builder.build_bert_example(source, target, semiotic_info, infer)
        if example is not None:
            yield example
            examples_count += 1

            if examples_count % 1000 == 0:
                logging.info("{} examples processed.".format(examples_count))
...
def read_input_file(input_filename: str) -> List[str]:
    """
    Args:
        input_filename (str): [description]
    ...
    with open(input_filename, 'r') as f:
        for line in f:
            if len(text_examples) % 1000 == 0:
                logging.info("{} examples processed.".format(len(text_examples)))
            text_examples.append(line)
    
    return text_examples
        

def create_dataset_batch_file(
        example_builder: 'BertExampleBuilder', file_size: int, input_filename: str, batch_file_directory: str, infer: bool = False
):
    """Create dataset batch file.

    Args:
        example_builder: Instance of BertExampleBuilder
        input_filename: Path to the TSV input file.
        batch_file_directory: Batch file directory
    ...
    base_output_filename = os.path.basename(input_filename) + "_part_{}.pkl"
    
    examples = []
    file_count = 0
    examples_count = 0
    logging.info(f"example file size: {file_size}")
    
    # directory file 검사부 세부 조건 추가 필요
    directory_files = os.listdir(batch_file_directory)
    if directory_files:
        ...
        for i, (source, target, semiotic_info) in enumerate(yield_sources_and_targets(input_filename)):
            if (i+1) % 1000 == 0:
                logging.info(f"{i+1} examples processed")
            example = example_builder.build_bert_example(source, target, semiotic_info, infer)
            examples.append(example)
            if len(examples) % file_size == 0:
                output_filename = os.path.join(batch_file_directory, base_output_filename.format(file_count))
                file_count += 1
                examples_count += file_size
                with open(output_filename, 'ab') as f_output:
                    logging.info(f"output_filename: {output_filename}")
                    pickle.dump(examples, f_output)
                del examples
                examples = []
        ...
        if examples:
            ...
            with open(output_filename, 'ab') as f_output:
                logging.info(f"{examples_count} examples processed. output_filename: {output_filename}")
                pickle.dump(examples, f_output)
            
            del examples
    
    file_name_list = natsort.natsorted(os.listdir(batch_file_directory), reverse=False)

    if "train" in os.path.basename(input_filename):
        file_name_list = [obj for obj in file_name_list if "train" in obj]
    elif "valid" in os.path.basename(input_filename):
        file_name_list = [obj for obj in file_name_list if "valid" in obj]
       
    gc.collect()
    return file_name_list, examples_count


def read_input_file_index(
        file_name_list: list,
    ...
    """Reads in Tab Separated Value file and converts to training/inference-ready examples by index.

    Args:
        file_name_list: File name list
        batch_file_directory: Batch file directory
        total_dataset_size: Total dataset size
    ...
    """
    batch_example = []
    last_batch_index = min(batch_index + batch_size, total_dataset_size)
    batch_file_list = file_name_list[batch_index:last_batch_index]
    
    for filename in batch_file_list:
        if filename.endswith(".pkl"):
            with open(os.path.join(batch_file_directory, filename), 'rb') as f:
                try:
                    example = pickle.load(f)
                    for ele in example:
                        batch_example.append(ele)
                except EOFError:
                    break

    return batch_example, last_batch_index
